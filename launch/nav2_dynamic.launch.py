...
def generate_launch_description():
    ...
    declare_map_cmd = DeclareLaunchArgument(
        'map', default_value=os.path.join(pkg_share, 'maps', 'baked_dynamic', 'map.yaml'),
        description='Occupancy map. Generate with bake_map.py, which slices the '
                    ...
                    '(wrong frame) or maps/005 (SLAM, shelves are hollow). Baked '
                    'from small_warehouse_dynamic at its INITIAL poses; the 11 '
                    'velocity-controlled props drift away from where the map shows '
                    'them, so the local costmap has to catch them at runtime.')
