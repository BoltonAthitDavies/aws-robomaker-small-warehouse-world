...
def _resource_path_actions():
    """Put the package share directory itself on the gz resource path.

    small_warehouse_static_tinymap.world includes its shell and props as
    `model://model_tiny/<model>` rather than `model://<model>`, because
    models/ now holds the RESCALED large warehouse and model_tiny/ holds the
    pristine small one -- a bare model name would silently load the wrong
    building. gz resolves the whole path after `model://` against each resource
    path entry, so naming a subdirectory only works if that subdirectory's
    PARENT is on the path; the package env-hook only adds models/ and worlds/.

    Prepended rather than appended so it is searched before models/. That costs
    nothing for the single-segment URIs still in the world (`ackermann_robot`,
    and the commented-out `sphere`): no such directory exists directly under
    the share dir, so the lookup falls straight through to models/.

    Added to the launch description BEFORE _override_speed_limits, which reads
    these variables back out of os.environ and prepends its own patched-model
    directory to whatever it finds -- so the speed patch still wins over both.
    """
    pkg_share = get_package_share_directory('aws_robomaker_small_warehouse_world')
    actions = []
    for var in ('IGN_GAZEBO_RESOURCE_PATH', 'GZ_SIM_RESOURCE_PATH'):
        existing = os.environ.get(var, '')
        actions.append(SetEnvironmentVariable(
            var, pkg_share + (os.pathsep + existing if existing else '')))
    return actions
...
def generate_launch_description():
    ...
    declare_world_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            pkg_share, 'worlds', 'small_warehouse_static', 'small_warehouse_static_tinymap.world'),
    ...
    # Must run BEFORE gz starts, and before _override_speed_limits, which reads
    # the resource path variables back from os.environ. See _resource_path_actions().
    for action in _resource_path_actions():
